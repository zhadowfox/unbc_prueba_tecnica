  <!-- Main modal -->
  <div id="modal-create-user" tabindex="-1" aria-hidden="true"
          ...
          <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
              ...
              <form class="text-white mx-4 group pb-8">
                  ...
                  <label class="my-1 lg:text-start text-center sm:w-full w-50 grid " for="email">
                      ...
                      <input type="text" name="email" wire:model.defer="email" id="email" placeholder=" "
                          class="focus:outline-none active:outline-none text-gray-900 rounded-md p-2 my-1 peer"
                          pattern="[a-z0-9._%+\-]+@[a-z0-9.\-]+\.[a-z]{2,}$" required />
                      <span
                          class="text-white text-sm hidden  peer-[&:not(:placeholder-shown):not(:focus):invalid]:block text-start">*Ingrese
                          un correo electronico</span>
                  </label>
                  ...
                  <label class="lg:text-start text-center grid" for="names">
                      ...
                      <input type="text" name="names" id="names" wire:model.defer="names"
                  ...
                  <label class="lg:text-start text-center grid" for="lastnames">
                      ...
                      <input type="text" name="lastnames" id="lastnames" wire:model.defer="lastnames"
                          class="md:w-full w-50 focus:outline-none active:outline-none text-gray-900 rounded-md p-2 my-1 flex-1 peer"
                          pattern=".{1,}$" required />
                      <span
                          class="text-white text-sm hidden peer-[&:not(:placeholder-shown):not(:focus):invalid]:block text-start">*Ingrese
                  ...
                  <label class="lg:text-start text-center grid" for="phone_number">
                      ...
                      <input type="number" name="phone_number" id="phone_number" wire:model.defer="phone_number"
                          class="md:w-full w-50 focus:outline-none active:outline-none text-gray-900 rounded-md p-2 my-1 flex-1 peer"
                          pattern="[0-9]*" required />
                      <span
                          class="text-white text-sm hidden peer-[&:not(:placeholder-shown):not(:focus):invalid]:block text-start">*Ingrese
                          un numero de telefono</span>
                  </label>
                  <div class="w-full flex">
                      <button type="button" wire:click="createUser" data-modal-hide="modal-create-user"
      ...
      <script>
          document.addEventListener('livewire:initialized', () => {
              @this.on('email-warning', (event) => {
                  const data = event
                  Swal.fire({
                      title: "Error",
                      icon: "error",
                      html: `El correo electronico ya existe:<br><b>Correo ingresado:${data[0]['email']} </b>`,
                      showCancelButton: false,
                      confirmButtonText: "Cerrar",
                      confirmButtonColor: "#2A4BA0",
                  })
              })
              @this.on('created-warning', (event) => {
                  const data = event
                  Swal.fire({
                      title: "success",
                      icon: "success",
                      html: `Usuario creado con exito: <br> <b>Correo:${data[0]['email']}<br>Nombres:${data[0]['names']}<br>Apellidos:${data[0]['lastnames']} </b>`,
                      showCancelButton: false,
                      confirmButtonText: "Cerrar",
                      confirmButtonColor: "#2A4BA0",
                  }).then(function(){
